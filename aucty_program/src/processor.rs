...
impl Processor {
  ...
  fn process_bid(accounts: &[AccountInfo], amount_expected_by_bidder: u64, program_id: &Pubkey) -> ProgramResult {
    let account_info_iter = &mut accounts.iter();
    ...
    msg!("Calling the token program to transfer tokens to the bidder...");
    invoke_signed(
